...
Probabilities GaussTransformDirect::compute(const Matrix& fixed,
    ...
    ///This is the old outlier_tmp used before
    double outlier_tmp =
        (outliers * moving.rows() * std::pow(-ksig * M_PI, 0.5 * cols)) /
        ((1 - outliers) * fixed.rows());
    
    //this is the outlier tmp used in corbot
//     double outlier_tmp = (std::pow(2*M_PI*sigma2, cols/2.0)/(1));
    //
	...
	pt1(i) = 1 - outlier_tmp / sp;
        ...
        //Calculation of l is unacessary atm
        //Instead its calculated in transform.hpp
        l += -std::log(sp);
    }
    //Calculation of l is unacessary atm
