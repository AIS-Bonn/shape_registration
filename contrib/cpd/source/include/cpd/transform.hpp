...
struct Result {
    ...
    /////this maybe doesnt belong here, necesarry to calculate l like corbot
//     Matrix W;
//     Matrix G;
    ///This destroys everything but the nonrigid transfornmation 
...
public:
    ...
    Result run(Matrix fixed, Matrix moving) {
        ...
        if (m_sigma2 == 0.0) {
            result.sigma2 = cpdG::default_sigma2(fixed, moving);
        } else if (m_normalize) {
        ...
        /////////////////
        //sigma2 is not sigma squared
        //result.sigma2 = result.sigma2 * result.sigma2;
        ////////////////
...
// 	    l = 0;
// 	    for(int n=0; n < fixed.rows(); n++){
// 		double distN = 0.0;
// 		for(int m=0; m < moving.rows(); m++){
// 		    Matrix g(1,3);
// 		    g(0,0) = 0;
// 		    g(0,1) = 0;
// 		    g(0,2) = 0;
// 		    for(int k=0; k < moving.rows(); k++){
// 			g = g + result.W.row(k) * result.G(k,m);
// 		    }
// 		    //std::cout << "g : " << g << std::endl;
// 		    double dist = ((fixed.row(n) - moving.row(m) - g).array().pow(2).sum())/result.sigma2;
// 		    distN += std::exp(-0.5 * dist);
// 		}
// 		l -= std::log(distN);
// 	    }
// 	    //std::cout << " l before trace beträgt: " << l << std::endl;
// 	    l = l + 3/2 * (result.W.transpose() * result.G * result.W).trace();
// 	    ntol = std::abs((l - L_old) / l);
	    
	    //not sure if needed
	    //probabilities.l = l;
	    //to caclulate the breaking condition
	    //L_old = l;
